...
export default function FeaturesPage() {
  ...
  const features = [
    {
      ...
      points: fr
        ? [
            "Eau Verte : eau de pluie consommée par évapotranspiration",
            "Eau Bleue : eau de surface et souterraine consommée",
            "Eau Grise : volume nécessaire pour diluer vos polluants jusqu'aux normes ambiantes",
            "Résultats exprimés en m³/unité, m³/an ou m³/tonne selon votre besoin",
          ]
        : [
            "Green Water: rainwater consumed through evapotranspiration",
            "Blue Water: surface and groundwater consumed",
            "Grey Water: volume needed to dilute pollutants to ambient standards",
            "Results in m³/unit, m³/year or m³/ton as needed",
          ],
      why: fr
        ? "Un calcul non conforme n'a aucune valeur auprès de vos auditeurs, clients ou partenaires. Avec HydroScan, votre empreinte est défendable — techniquement et réglementairement."
        : "A non-compliant calculation has no value for your auditors, clients or partners. With HydroScan, your footprint is defensible — technically and regulatorily.",
    ...
    },
    {
      ...
      points: fr
        ? [
            "Interface de saisie guidée, étape par étape",
            "Sélection automatique des données climatiques (CLIMWAT/CROPWAT FAO)",
            "Calcul en temps réel dès la dernière saisie",
            "Résultats disponibles immédiatement, sans file d'attente",
          ]
        : [
            "Step-by-step guided input interface",
            "Automatic climate data selection (CLIMWAT/CROPWAT FAO)",
            "Real-time calculation from last input",
            "Results available immediately, no queue",
          ],
      why: fr
        ? "Le temps de vos équipes est précieux. HydroScan supprime la friction entre la décision de mesurer et l'obtention d'un résultat fiable."
        : "Your teams' time is valuable. HydroScan removes friction between the decision to measure and getting a reliable result.",
      color: "text-primary",
    },
    {
  ...
  const pillars = [
    ...
    },
    {
      ...
      badge: "🌍",
      title: fr ? "Données de référence internationales" : "International reference data",
      text: fr
        ? "HydroScan intègre les bases de données officielles : CROPWAT 8.0, CLIMWAT 2.0, FAOSTAT, AQUASTAT, ainsi que les normes de qualité eau régionales (EU WFD, US EPA, WHO). Vos calculs s'appuient sur les mêmes sources que les chercheurs et auditeurs."
        : "HydroScan integrates official databases: CROPWAT 8.0, CLIMWAT 2.0, FAOSTAT, AQUASTAT, and regional water quality standards (EU WFD, US EPA, WHO). Your calculations rely on the same sources as researchers and auditors.",
    },
    {
      ...
      <section className="pt-32 pb-20 px-[5%] text-center bg-card">
        <motion.div initial="hidden" animate="visible" variants={stagger} className="mx-auto max-w-[800px]">
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-4 py-1.5 text-xs font-semibold text-primary mb-7">
            ...
            {fr ? "Plateforme complète" : "Complete platform"}
          </motion.div>
          ...
          <motion.h1 variants={fadeUp} className="font-display text-[clamp(2.4rem,5vw,4rem)] font-extrabold leading-[1.1] text-foreground mb-5">
            {fr ? (
              <>Tout ce dont vous avez besoin pour <span className="text-primary">mesurer & agir</span></>
            ) : (
              <>Everything you need to <span className="text-primary">measure & act</span></>
            )}
          </motion.h1>

          <motion.p variants={fadeUp} className="text-lg text-muted-foreground max-w-[620px] mx-auto mb-10">
            {fr
              ? "De l'analyse rapide au rapport ISO certifié, HydroScan couvre l'ensemble de vos besoins environnementaux — en quelques minutes, sans expertise technique préalable."
              : "From quick analysis to certified ISO reports, HydroScan covers all your environmental needs — in minutes, with no prior technical expertise."}
          </motion.p>
        ...
        <div className="mx-auto max-w-[1100px]">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
            ...
            <h2 className="font-display text-[clamp(1.8rem,3.5vw,2.8rem)] font-extrabold text-foreground leading-tight">
              {fr ? (
                <>Six outils. Un seul objectif :<br /><span className="text-primary">réduire votre empreinte eau.</span></>
              ) : (
                <>Six tools. One goal:<br /><span className="text-primary">reduce your water footprint.</span></>
              )}
            </h2>
          </motion.div>
